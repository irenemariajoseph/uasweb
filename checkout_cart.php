<?php
    ...
    while($cart_data = $result_cart->fetch()) {
        ...
        $query_insert_transaksi = "INSERT INTO 
                tbl_pesan(prod_id, user_id, username, email, no_telp, jenis_pemb, cara_pickup, tgl_pickup, jam_pickup, jmlh_brg, subtotal, pajak, harga_total)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $prepare_insert_transaksi = $connInsertTransaksi->prepare($query_insert_transaksi);
        $prepare_insert_transaksi->execute([$cart_data['prod_id'], 
                                                $user_data['id'], 
                                                $user_data['username'], 
                                                $user_data['email'], 
                                                $user_data['no_telp'], 
